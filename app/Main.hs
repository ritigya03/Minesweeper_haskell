...
tileClicked window rows cols grid buttons (i, j) = do
    ...
    case currentGrid !! i !! j of
        Mine -> do
            ...
            postGUIAsync $ putStrLn "Oh Duck! Game Over! Don't die so soon the next time"
            -- postGUIAsync $ set window [windowTitle := ("Game Over" :: String)]
            set window [windowTitle := ("Oh Duck! Game over" :: String)]
    ...
    if checkWin currentGrid' 
        then do
            ...
            set window [windowTitle := ("You WIN! Let's DUCKING GO!!" :: String)]
            postGUIAsync $ putStrLn "You WIN! Let's DUCKING GO!!"
            _ <- timeoutAdd (widgetDestroy window >> return False) 3000 
            return ()
        else return ()
